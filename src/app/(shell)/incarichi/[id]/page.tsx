...
import { tasks, clients, subtasks } from "@/lib/db";
...
export default async function IncaricoDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const client = task.client_id ? clients.getById(task.client_id) : undefined
  const taskSubtasks = subtasks.getByTask(id)
        ...
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap items-center gap-3">
              {client?.name && (
                <span className="text-sm font-medium text-primary/70">{client.name}</span>
              )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-xl border border-border p-6">
            ...
            <SubtaskList taskId={task.id} initialSubtasks={taskSubtasks} />
          </div>
        </div>
